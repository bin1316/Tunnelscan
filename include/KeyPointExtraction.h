...
namespace pcl
{
	...
	class KeyPointExtaction : public FilterIndices<PointT>
	{
	protected:
	...
	public:
		...
		*/
		KeyPointExtaction(bool extract_removed_indices = false) :
			FilterIndices<PointT>::FilterIndices(extract_removed_indices),
			searcher_(),
			mean_k_(1),
			std_mul_(0.0)
		{
				filter_name_ = "KeyPointExtaction";
			}
		...
		*/
		inline void
			getKeyIndice(std::vector<int> &gt_indices)
		{
				gt_indices = indices;
			}
...
pcl::KeyPointExtaction<PointT>::applyFilter(PointCloud &output)
{
	indices.clear();
	if (keep_organized_)
	{
...
template <typename PointT> void
pcl::KeyPointExtaction<PointT>::applyFilterIndices(std::vector<int> &indices)
{
